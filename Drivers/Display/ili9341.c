    ...
    if ( (NULL != reset_port)        && IS_GPIO_PIN(reset_pin)         &&
      ...
      if ( itsSupported != touch_support ||
        ...
        if (NULL != (lcd = malloc(sizeof(ili9341_t)))) {
          ...
          if (touch_support) {
        	lcd->touch_spi_hal              = touch_spi_hal;
    ...
    uint16_t *x_pos, uint16_t *y_pos)
{
  ...
  // TODO: based on STM32G4, which is clocked at 170MHz. support other chips. DOnt Need to own SPI
  //MODIFY_REG(lcd->spi_hal->Instance->CR1, SPI_CR1_BR, SPI_BAUDRATEPRESCALER_128);
  /*GPIO_InitTypeDef GPIO_InitStruct = {0};
  GPIO_InitStruct.Pin = lcd->touch_irq_pin;
  GPIO_InitStruct.Mode = GPIO_MODE_INPUT;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  HAL_GPIO_Init(lcd->touch_irq_port, &GPIO_InitStruct);*/
...
void ili9341_spi_touch_select(ili9341_t *lcd)
{
  // clear bit indicates the touch screen is -active- slave SPI device
	HAL_GPIO_DeInit(lcd->touch_irq_port,lcd->touch_irq_pin);
	 GPIO_InitTypeDef GPIO_InitStruct = {0};
		GPIO_InitStruct.Pin = lcd->touch_irq_pin;
	    GPIO_InitStruct.Mode = GPIO_MODE_INPUT;
	    GPIO_InitStruct.Pull = GPIO_NOPULL;
...
void ili9341_fill_block(ili9341_t *lcd, uint16_t *buf,
    int16_t x, int16_t y, uint16_t x2, uint16_t y2)
{
  // verify we have something within screen dimensions to be drawn
  /*if (ibNOT(ili9341_clip_rect(lcd, &x, &y, &x2, &y2)))
    { return; }*/

  uint32_t num_pixels = ((x2-x)+1) * ((y2-y)
		  +1);
 // uint32_t rect_wc    = num_pixels;

  // select target region
  ili9341_spi_tft_set_address_rect(lcd, x, y, x2, y2);
  ...
  while (rect_wc > 0) {
    ...
    if (curr_wc > block_wc)
      { curr_wc = block_wc; }
    ili9341_transmit_color(lcd, curr_wc * 2, spi_tx_block, ibYes);
    rect_wc -= curr_wc;
  }*/
  ili9341_transmit_color(lcd, num_pixels * 2/*16-bit words*/, buf, ibYes);
